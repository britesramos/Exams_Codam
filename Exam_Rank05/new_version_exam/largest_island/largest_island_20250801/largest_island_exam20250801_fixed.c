...
char* ft_strjoin(char *buffer, char* temp)
{
    ...
    res[i + j] = '\0'; // Always null terminate //DIF
    free(buffer); // Free the old buffer //DIF
    return (res); 
}
...
char* read_file(int fd)
{
    ...
    while (bytes_read == 5) //DIF // Only continue if buffer was full
    {
        bytes_read = read(fd, temp, 5);
        if (bytes_read <= 0) //DIF
            break ;
        temp[bytes_read] = '\0'; // Null terminate temp //DIF
...
char** parse_map(char *buffer)
{
    ...
    map[k] = NULL; // Properly terminate the array //DIF
    return (map);
}

int rec_flood_fill(int rows, int col, int x, int y, char **map)
{
    if (x >= col || y >= rows || x < 0 || y < 0 || (map[y][x] != 'X')) //DIF
        return (0);
...
void floodfill(char *buffer, char **map)
{
    ...
    while(map[y])
    {
        while (map[y][x] && x < col) // Add boundary check //DIF
        {
            if (map[y][x] == 'X'){
