...
void broadcast_message(struct t_data *data, char *str, int client_fd){
	for (int i = 0; data->clients[i].fd != -1; i++){
		if (data->clients[i].fd != client_fd && data->clients[i].fd != -1){
			send(data->clients[i].fd, str, strlen(str), 0);
		}
	}
}
...
int main_loop(struct t_data *data){
	...
	while (1){
		...
		if(FD_ISSET(data->server_fd, &set_fds)){
			// 1. Call accept() to get the new_client_fd.
		...
		for (int i = 0; data->clients && data->clients[i].fd != -1; i++){
			if (FD_ISSET(data->clients[i].fd, &set_fds)){
				int bytes_read = recv(data->clients[i].fd, buffer, 999, 0);
				if (bytes_read <= 0){
